...
class MembraneData(BaseModel):
    ...
    Substrate_pore_size: Optional[Union[float, str]] = Field(
        None, description="支撑层孔径均值 (nm)，仅保留均值，不含误差"
    )
    Substrate_MWCO: Optional[Union[float, str]] = Field(
        None, description="支撑层截留分子量均值 (kDa)，仅保留均值"
    )
    Substrate_Water_contact_angle: Optional[Union[float, str]] = Field(
        None, description="支撑层水接触角均值 (°)，仅保留均值"
    )
    Substrate_zeta: Optional[Union[float, str]] = Field(
        None, description="支撑层 zeta 电位均值 (mV)，仅保留均值"
    )
    Substrate_Ra: Optional[Union[float, str]] = Field(
        None, description="支撑层粗糙度均值 (nm)，仅保留均值"
    )
    ...
    Degree_of_crosslinking: Optional[Union[float, str]] = Field(
        None, description="O/N 交联度均值（无量纲），仅保留均值"
    )
    Thickness: Optional[Union[float, str]] = Field(
        None, description="皮层厚度均值 (nm)，仅保留均值"
    )
    Effective_pore_size: Optional[Union[float, str]] = Field(
        None, description="有效孔径均值 (nm)，仅保留均值"
    )
    Zeta_potential: Optional[Union[float, str]] = Field(
        None, description="皮层 zeta 电位均值 (mV)，仅保留均值"
    )
    Membrane_Ra: Optional[Union[float, str]] = Field(
        None, description="分离层粗糙度均值 (nm)，仅保留均值"
    )
